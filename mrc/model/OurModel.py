...
from mrc.layers.OurLayer import  regularizer, residual_block, highway, conv, mask_logits, trilinear, total_params, position_embedding
from mrc.model.Optimizer import AdamWOptimizer
from tensorflow.python.ops import array_ops

from mrc.utils.dureader_eval import compute_bleu_rouge
from mrc.utils.dureader_eval import normalize
...
class Model(object):
    ...
    def find_best_answer_with_score(self, sample, start_prob, end_prob, padded_p_len):
        """
        Finds the best answer for a sample given start_prob and end_prob for each position.
        This will call find_best_answer_for_passage because there are multiple passages in a sample
        """
    ...
    def forward_one_sample(self, sample):
        # for sample in batch_data:
        feed_dict = {self.c: sample['passage_token_ids'],
                     self.q: sample['question_token_ids'],
                     self.qh: sample['question_char_ids'],
                     self.ch: sample["passage_char_ids"],
                     self.start_label: sample['start_id'],
                     self.end_label: sample['end_id'],
                     self.dropout: 0.0}

        start_probs, end_probs, loss = self.sess.run([self.logits1,
                                                      self.logits2, self.loss], feed_dict)

        padded_p_len = len(sample['passage_token_ids'][0])
        pred_answers = []
        for sample, start_prob, end_prob in zip(sample['raw_data'], start_probs, end_probs):

            best_answer, score = self.find_best_answer_with_score(sample, start_prob, end_prob, padded_p_len)
            # if save_full_info:
            sample['pred_answers'] = [best_answer]
            sample['pred_score'] = [score]
            pred_answers.append(sample)
